...
static struct vb2_buffer *queued_bufs[MYV4L2_MAX_BUFS];
static int queued_head, queued_tail, queued_count;
static struct task_struct *capture_thread;
...
{
    if (vb2_plane_size(vb, 0) < MYV4L2_FRAME_SIZE)
        return -EINVAL;
    vb2_set_plane_payload(vb, 0, MYV4L2_FRAME_SIZE);
    return 0;
}
...
static void buffer_queue(struct vb2_buffer *vb)
{
    ...
    if (queued_count < MYV4L2_MAX_BUFS) {
        queued_bufs[queued_tail] = vb;
        queued_tail = (queued_tail + 1) % MYV4L2_MAX_BUFS;
        queued_count++;
    } else {
        printk(KERN_ERR "myv4l2: internal buffer tracking full, dropping buffer\n");
    }
    spin_unlock_irqrestore(&buffer_lock, flags);
}

static int capture_thread_fn(void *data)
{
    struct vb2_buffer *vb;
    void *vaddr;
    unsigned long flags;
    static u8 fill_value;

    printk(KERN_INFO "myv4l2: capture thread started\n");
    ...
    while (!kthread_should_stop()) {
        ...
        spin_lock_irqsave(&buffer_lock, flags);
        if (queued_count > 0) {
            vb = queued_bufs[queued_head];
            queued_head = (queued_head + 1) % MYV4L2_MAX_BUFS;
            queued_count--;
        }
        spin_unlock_irqrestore(&buffer_lock, flags);
        ...
        if (vb) {
            ...
            if (vaddr) {
                memset(vaddr, fill_value, MYV4L2_FRAME_SIZE);
                fill_value += 10;
            }
            vb2_set_plane_payload(vb, 0, MYV4L2_FRAME_SIZE);
    ...
    printk(KERN_INFO "myv4l2: capture thread stopping\n");
    return 0;
}

static int start_streaming(struct vb2_queue *vq, unsigned int count)
{
    printk(KERN_INFO "myv4l2: start_streaming called\n");
    capture_thread = kthread_run(capture_thread_fn, NULL, "myv4l2_capture");
    if (IS_ERR(capture_thread)) {
        int err = PTR_ERR(capture_thread);
        printk(KERN_ERR "myv4l2: failed to start capture thread: %d\n", err);
        capture_thread = NULL;
        return err;
    }
    return 0;
...
static void stop_streaming(struct vb2_queue *vq)
{
    ...
    printk(KERN_INFO "myv4l2: stop_streaming called\n");

    if (capture_thread) {
        kthread_stop(capture_thread);
        capture_thread = NULL;
    }
...
MODULE_DESCRIPTION("Day 11 - STREAMON/STREAMOFF with synthetic frame generator kthread");
